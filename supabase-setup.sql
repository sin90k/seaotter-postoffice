...
-- 补充字段以支持完整审计：类型（加/减）、余额、引用 ID、备注、操作者。
...
-- 放宽并统一 source 的取值范围，兼容旧值（registration/postcard/admin_adjust/event）
ALTER TABLE public.credits_ledger
  DROP CONSTRAINT IF EXISTS credits_ledger_source_check;

ALTER TABLE public.credits_ledger
  ADD CONSTRAINT credits_ledger_source_check
  CHECK (
    source IN (
      'generation_cost',
      'purchase',
...
CREATE OR REPLACE FUNCTION public.update_user_credits(
  ...
  p_bucket text DEFAULT NULL -- 'promo' | 'paid' | NULL（按规则自动）
)
...
BEGIN
  ...
  -- 加锁读取当前积分
  SELECT promo_credits, paid_credits
  INTO v_promo, v_paid
  ...
  IF p_amount < 0 THEN
    -- 扣减积分：默认先扣 promo，再扣 paid
    v_need := -p_amount;
    IF p_bucket = 'promo' THEN
      IF v_promo < v_need THEN
        RAISE EXCEPTION 'insufficient promo credits';
      END IF;
      v_promo := v_promo - v_need;
    ELSIF p_bucket = 'paid' THEN
      IF v_paid < v_need THEN
        RAISE EXCEPTION 'insufficient paid credits';
      END IF;
      v_paid := v_paid - v_need;
    ELSE
      IF v_promo + v_paid < v_need THEN
        RAISE EXCEPTION 'insufficient total credits';
      END IF;
      IF v_promo >= v_need THEN
        v_promo := v_promo - v_need;
      ELSE
  ...
  ELSE
    -- 增加积分：按来源或 bucket 决定加到 promo 还是 paid
    IF p_bucket = 'promo' THEN
      v_promo := v_promo + p_amount;
    ELSIF p_bucket = 'paid' THEN
      v_paid := v_paid + p_amount;
    ELSE
      IF p_source IN ('signup_bonus', 'promo_reward') THEN
        v_promo := v_promo + p_amount;
      ELSE
        v_paid := v_paid + p_amount;
      END IF;
  ...
  -- 更新用户积分汇总（购买时累计 total_paid_credits）
  UPDATE public.profiles
  SET
    promo_credits = v_promo,
    paid_credits = v_paid,
    credits = v_total,
    total_paid_credits = CASE
      WHEN p_source = 'purchase' AND p_amount > 0 THEN COALESCE(total_paid_credits, 0) + p_amount
  ...
  INSERT INTO public.credits_ledger (
    ...
    p_user_id,
    CASE
      WHEN p_bucket = 'promo' OR p_source IN ('signup_bonus', 'promo_reward') THEN 'promo'
      ELSE 'paid'
    END,
    p_source,
    p_amount,
